...
namespace LinqExercise
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var sortNumDes = numbers.OrderByDescending(number => number);
            foreach (var number in sortNumDes) 
            {
                Console.WriteLine(number);
            }
            ...
            var orderedNums = numbers.OrderBy(number => number);
            foreach (var number in orderedNums.Take(4)) 
            {
                Console.WriteLine($"{number}");
            }
            //TODO: Change the value at index 4 to your age, then print the numbers in descending order
            Console.WriteLine("Adding age descending.");
            numbers[4] = 36;
            
            var numbersDesc = numbers.OrderByDescending(number => number);
            ...
            var specialFilterEmployees = employees.Where(person => person.YearsOfExperience <= 10 && person.Age > 35);

            Console.WriteLine($"Total years of experience {specialFilterEmployees.Sum(x => x.YearsOfExperience)}");

            Console.WriteLine($"Average years of experience {specialFilterEmployees.Average(x => x.YearsOfExperience)} ");
            //TODO: Add an employee to the end of the list without using employees.Add()
            employees = employees.Append(new Employee("John", "Ward", 32, 6 )).ToList();
